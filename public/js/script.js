console.log("New JS file, POS1.js");
localStorage.clear();
...
function saveStaticDataToFile() {
    ...
    myArray.forEach(sale => {
        ...
        localStorage.setItem(`sold${sale.id}`, 0);
...
try {
    myArray.forEach(data => {
        menu12.innerHTML += `<div id="${data['id']}" class="item" data-bs-toggle="popover" title="ID :  ${data['id']}" data-bs-content=" " value="${data['name']}"><div class='col'><div class="card" style="height:200px; width: 200px;">
            <div class="card-image">
            <img src="${data['image_link']}" style="height:120px; width: 200px;"></a>
            </div>
            <div class="card-content">   
            <div class="container"><pre><h5 style="text-align: center;" id='name${data['id']}'>${data['name']}</h5><span style="text-align: left; color:red;" id='stock${data['id']}'>${String(data['stock'])}</span> <h6 style="text-align: right" id='price${data['id']}'>${"        SAR " + String(data['price'])}</h6></pre></div><div class="card-action"></a></div>
            </div>
            </div>
            </div></div>`
    });
...
//     if (access == "false") {
//         document.getElementById('intimation').innerText = "INCORRECT Username OR Password,        Please Try Again"
//     } else if (access == "true") {
//         document.getElementById("title12").innerText = username
//         document.getElementById("modal6").hidden = true;
//         location.reload()
//     }
...
proceed_btn7_onclick = () => {
    console.log("button proceed 5 working fine");

    let pin = Number(localStorage.getItem("adminpin"));

    let inputPin = document.getElementById("inputGroup-sizing-lg7").value;
    ...
    if (inputPin == pin) {
        document.getElementById("modal-body7").innerHTML = `<form class="row g-3">
      <div class="col-md-6">
        <label for="inputEmail4" class="form-label">Username</label>
        <input type="text" class="form-control" id="inputUsername12" value="">
      </div>
      ...
      <label for="inputAddress" class="form-label" id="warning1" style="color: red;"></label>
      <div class="col-12">
        ...
        document.getElementById("modal-footer7").innerHTML = `<button type="button" class="btn btn-success" onclick="add_User_new()">Save</button>`
    }
}

const add_User_new = () => {
    let inputusername = document.getElementById("inputUsername12").value;
    let pass1 = document.getElementById("inputpassword1").value;
    let pass2 = document.getElementById("inputpassword2").value;
    let mylabel = document.getElementById("warning1");

    if (pass1 != pass2) {
        mylabel.innerText = "Passwords Don't match,  please enter both passwords again"
        return
    } else {
        ...
        if (newuser == true) {
            let addnew = { "username": inputusername, 'password': pass1 };
            users2.push(addnew);
            localStorage.setItem('users', JSON.stringify(users2));

            document.getElementById("modal7").hidden = true;
            location.reload()
        }
...
async function main() {
    ...
    // let users = [{ "username": "Muhammad Mohib", "password": "mohib122" }];
    // localStorage.setItem('users', JSON.stringify(users));

    let sessionAccess = sessionStorage.getItem("access");
    if (sessionAccess !== 'true') {
        btn = document.getElementById("loginButton");
        btn.click()
    }

    localStorage.setItem("adminpin", "1227292");

    document.getElementById("title12").innerText = localStorage.getItem("currentuser");
